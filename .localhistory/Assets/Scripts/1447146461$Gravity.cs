...
public class Gravity : MonoBehaviour {
	...
	void Update () 
    {
        Vector3 raycastStart = transform.position - (Vector3.up * transform.lossyScale.y);
        ...
        if (_falling)
        {
            ...
            if (hit.collider != null)
            {
                ...
                transform.position = new Vector3(transform.position.x, hit.point.y + height + 0.02, transform.position.z);
            }
            else
            {
        ...
        RaycastHit2D checkIfFalling = Physics2D.Raycast(raycastStart, -Vector2.up, 0.05f);
