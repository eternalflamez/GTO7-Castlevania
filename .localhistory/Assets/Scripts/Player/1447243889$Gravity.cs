...
public class Gravity : MonoBehaviour {
	...
	void Update () 
    {
        ...
        RaycastHit2D checkIfFallingLeft = Physics2D.Raycast(raycastStart + (Vector3.left * (width - 0.01f)), -Vector2.up, .1f);
        RaycastHit2D checkIfFallingRight = Physics2D.Raycast(raycastStart + (Vector3.right * (width - 0.01f)), -Vector2.up, .1f);
